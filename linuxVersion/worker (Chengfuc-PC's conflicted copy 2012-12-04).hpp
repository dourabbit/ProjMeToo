...
//        }
    ...
    int GlobalIndex2Local(const int globalIndex) const{
        int localIndex = -1;
		const int	o_x = this->pos.x,
					o_y = this->pos.y,
					rend_w = this->wholeBlock->width,
					rend_h = this->wholeBlock->height,
					global_col = globalIndex % rend_h,
					global_row = globalIndex/rend_h,
					c = global_col - o_x,
					r = global_row -o_y;	
		localIndex = r*this->width+c;
        return localIndex;
